...
with st.sidebar:
    ...
    st.caption("Approved engineering target first; otherwise historical percentiles.")
    st.markdown(
...
with tab_process:
    ...
    with c1:
        if "AEQ_COD" in df.columns:
            st.plotly_chart(line_chart(recent_df, "AEQ_COD", "AEQ COD concentration", "AEQ_COD_MA7"), use_container_width=True)
    with c2:
        if "COD_Load_Recalc_kg_d" in df.columns:
            st.plotly_chart(line_chart(recent_df, "COD_Load_Recalc_kg_d", "Biological COD load", "COD_Load_Recalc_kg_d_MA7"), use_container_width=True)
    ...
    st.plotly_chart(line_chart(recent_df, selected_nutrient), use_container_width=True)
    ...
    st.plotly_chart(line_chart(recent_df, stage_metric), use_container_width=True)
...
with tab_bio:
    ...
    with c1:
        if "EOA_SS" in df.columns:
            st.plotly_chart(line_chart(recent_df, "EOA_SS", "MLSS", "EOA_SS_MA7"), use_container_width=True)
    with c2:
        if "EOA_MLVSS" in df.columns:
            st.plotly_chart(line_chart(recent_df, "EOA_MLVSS", "MLVSS", "EOA_MLVSS_MA7"), use_container_width=True)
...
with tab_history:
    ...
    if valid_lag.empty:
        ...
        st.plotly_chart(best_scatter, use_container_width=True)
