# -*- coding: utf-8 -*-
"""
WWTP Performance & Diagnostic Platform — V1.3
============================================
...
PROCESS_DIAGRAM_NODES = [
    {
        ...
        "x": 0.0, "y": 0.80, "w": 0.72, "h": 0.20,
        "core": ["PM1_Flow", "PM1_COD", "PM1_pH"],
    ...
    },
    {
        ...
        "x": 0.0, "y": 0.50, "w": 0.72, "h": 0.20,
        "core": ["PM2_Flow", "PM2_COD", "PM2_pH"],
    ...
    },
    {
        ...
        "x": 0.0, "y": 0.20, "w": 0.72, "h": 0.20,
        "core": ["PM3_Flow", "PM3_COD", "PM3_pH"],
    ...
    },
    {
        ...
        "x": 1.25, "y": 0.50, "w": 0.92, "h": 0.32,
        "core": ["PM_Total_Flow", "PM_COD", "PM_COD_Load_Recalc_kg_d", "PM_pH"],
        "details": ["PM_Total_Flow", "PM_pH", "PM_SS", "PM_COD", "PM_TN", "PM_SS_Load_Excel", "PM_COD_Load_Recalc_kg_d"],
    },
    {
        "id": "AEQ", "name": "AEQ", "subtitle": "Equalization basin",
        "x": 2.55, "y": 0.50, "w": 0.92, "h": 0.32,
        "core": ["AEQ_COD", "COD_Load_Recalc_kg_d", "AEQ_TN", "AEQ_TP"],
    ...
    },
    {
        ...
        "x": 3.85, "y": 0.50, "w": 0.92, "h": 0.32,
        "core": ["N_COD_g_kg", "P_COD_g_kg", "DN_Outlet_COD", "Internal_Recycle_Ratio_Recalc"],
        "details": ["DN_Inlet_pH", "DN_Inlet_SS", "DN_Inlet_TN", "DN_Inlet_NO3N", "DN_Inlet_30min_Settling", "DN_Outlet_pH", "DN_Outlet_SS", "DN_Outlet_NH3N", "DN_Outlet_NO3N", "DN_Outlet_COD", "DN_TN_Inlet_Recalc_mg_L", "N_COD_g_kg", "P_COD_g_kg", "Nutrient_N_kg_d", "Nutrient_P_kg_d", "Internal_Recycle_Ratio_Recalc", "Stage_I_HRT_h", "Stage_I_Volumetric_COD_Load_kg_m3_d", "Stage_I_Outlet_COD_Load_kg_d", "DN_Apparent_COD_Removal_pct"],
    },
    {
        "id": "AER", "name": "Aeration Tank", "subtitle": "Independent Aeration II",
        "x": 5.15, "y": 0.50, "w": 1.00, "h": 0.32,
        "core": ["EOA_SS", "F_M_Total_Bio_7d", "SRT_TSS_7d_Recalc_d", "Aeration_Inlet_Temperature"],
        "details": ["EOA_pH", "EOA_SS", "EOA_MLVSS", "MLVSS_MLSS_Ratio_Recalc", "F_M_Total_Bio_kgCOD_kgMLVSS_d", "F_M_Total_Bio_7d", "SRT_TSS_Daily_Recalc_d", "SRT_TSS_7d_Recalc_d", "SRT_TSS_14d_Recalc_d", "Aeration_Inlet_Temperature", "Online_MLSS_1", "Online_MLSS_2", "Aeration_II_HRT_h", "Aeration_II_Volumetric_COD_Load_kg_m3_d"],
    },
    {
        "id": "SEC", "name": "Secondary", "subtitle": "Clarification / sludge",
        "x": 6.55, "y": 0.50, "w": 1.02, "h": 0.32,
        "core": ["EOA_SVI", "SC_Sludge_Blanket", "RS_Ratio_Excel", "ES_TSS_Removed_kg_d"],
        "details": ["EOA_30min_Settling", "EOA_SVI", "SVI_Recalc_mL_g", "SC_Sludge_Blanket", "RS_Flow", "RS_SS", "RS_SS_Online", "RS_Ratio_Excel", "RS_Ratio_Recalc", "RAS_TSS_Circulation_kg_d", "ES_Flow", "ES_SS", "ES_SS_Online", "ES_TSS_Removed_kg_d", "ES_VSS_Removed_kg_d_Est", "ES_Removal_kgVSS_kgCOD_Est", "DryCake_DrySolids_t_d_Est"],
    },
    {
        "id": "EO", "name": "EO", "subtitle": "Final effluent",
        "x": 7.92, "y": 0.50, "w": 0.88, "h": 0.32,
        "core": ["EO_COD", "EO_TN", "EO_NH3N", "EO_TP"],
...
def _node_hover_text(node: dict, current_date: pd.Timestamp, ref_df: pd.DataFrame) -> str:
    """Rich hover text: latest value, unit, 7-day average and status basis."""
    history = df.loc[df["Date"] <= current_date]
    lines = [f"<b>{html.escape(node['name'])}</b>", html.escape(node['subtitle']), ""]
    detail_cols = available(node.get("details", []))
    for col in detail_cols:
        val, dt = latest_non_null(history, col)
        if pd.isna(val):
            continue
        status, reason = calc_status(col, val, ref_df)
        ...
        avg7 = seven_day_average(df, col, current_date)
        avg_txt = f" · 7d {fmt_value(avg7, col)}{unit_txt}" if pd.notna(avg7) else ""
        date_txt = dt.strftime("%Y-%m-%d") if pd.notna(dt) else "—"
        lines.append(
            f"<b>{html.escape(humanize(col))}</b>: {fmt_value(val, col)}{unit_txt}{avg_txt}"
            f"<br><span style='color:#667085'>{html.escape(label)} · {html.escape(reason)} · latest {date_txt}</span>"
        )
    if len(lines) <= 3:
        lines.append("No valid data available for this stage.")
    return "<br>".join(lines)


def _node_visible_text(node: dict, current_date: pd.Timestamp) -> str:
    """Keep only a few important values on the diagram; details remain in hover."""
    history = df.loc[df["Date"] <= current_date]
    lines = [f"<b>{html.escape(node['name'])}</b>"]
    # Small PM inlet nodes need fewer lines to stay readable.
    ...
    for col in available(node.get("core", []))[:max_metrics]:
        ...
        unit = unit_of(col)
        unit_txt = f" {html.escape(unit)}" if unit not in {"", "-"} else ""
        lines.append(f"{html.escape(humanize(col))}: {fmt_value(val, col, compact=True)}{unit_txt}")
    return "<br>".join(lines)
...
def process_schematic_figure(current_date: pd.Timestamp, ref_df: pd.DataFrame) -> go.Figure:
    ...
    connections = [
        ...
        ("PM", "AEQ"), ("AEQ", "DN"), ("DN", "AER"), ("AER", "SEC"), ("SEC", "EO"),
    ]
    ...
    # Draw nodes as neutral process equipment; status is encoded by border color.
    for node in PROCESS_DIAGRAM_NODES:
        ...
        fig.add_annotation(
            x=node["x"], y=node["y"],
            text=_node_visible_text(node, current_date),
            showarrow=False,
            align="center",
    ...
    fig.add_annotation(x=4.55, y=0.95, text="Biological treatment", showarrow=False, font=dict(size=11, color="#667085"))
    fig.add_annotation(x=7.25, y=0.95, text="Separation / effluent", showarrow=False, font=dict(size=11, color="#667085"))

    fig.update_xaxes(visible=False, range=[-0.55, 8.50], fixedrange=True)
...
with tab_current:
    ...
    st.caption(
        "Schematic process view: PM1/PM2/PM3 → Mixed PM → AEQ → D/N (including Aeration I) "
        "→ Aeration Tank → Secondary → EO. Hover over a process node to see its detailed latest values, units, 7-day averages and status basis."
    )
    st.markdown(
    ...
    with st.expander("Process stage detail — all available metrics", expanded=False):
        stage_choice = st.selectbox(
            "Stage",
            [n["name"] for n in PROCESS_DIAGRAM_NODES],
            key="stage_detail_v13",
        )
        stage = next(n for n in PROCESS_DIAGRAM_NODES if n["name"] == stage_choice)
        st.dataframe(
            process_stage_rows(stage, analysis_date, status_reference),
            use_container_width=True,
            hide_index=True,
        )
